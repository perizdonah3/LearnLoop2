...
fun AppNavHost(
    ...
    NavHost(
        ...
        composable(ROUT_INTRO) { IntroScreen(navController) }
        ...
        // 📚 CORE FEATURES
        composable(ROUT_PLANNER) { PlannerScreen(navController) }
        composable(ROUT_NOTEVAULT) { NoteVaultScreen(navController) }

        composable(ROUT_STUDYMATERIALS) { IntentStudyMaterialsScreen(navController) }
        composable(ROUT_PROGRESSTRACKER) { ProgressTrackerScreen(navController) }

        composable(ROUT_QUIZ) { QuizScreen(navController) }
        composable(ROUT_QUIZRESULTS) { QuizResultsScreen(navController) }

        composable(ROUT_STUDYTIMER) { StudyTimerScreen(navController) }

        composable(ROUT_SUBJECT) { SubjectScreen(navController) }
        composable(ROUT_SUBJECTDETAILS) { SubjectDetailsScreen(navController) }

        // 🌍 EXTRA FEATURES
        composable(ROUT_CAREERGUIDANCE) { CareerHubScreen(navController) }
        composable(ROUT_STUDENTCOMMUNITY) { StudentCommunityScreen(navController) }
        composable(ROUT_WELLNESS) { WellnessScreen(navController) }

        composable(ROUT_ACHIEVEMENTS) { AchievementScreen(navController) }
        composable(ROUT_STUDYSTREAK) { StudyStreakScreen(navController) }
    }
}